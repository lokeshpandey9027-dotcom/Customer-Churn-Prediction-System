...
db = client.customer_churn_database
...
async def login_post(response: Response, request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if valid_user:
        ...
        return templates.TemplateResponse(request=request, name="login.html", context={"error": "Invalid username or password", "success": None})

@app.post("/register", response_class=HTMLResponse)
async def register_post(request: Request, reg_username: str = Form(...), reg_email: str = Form(...), reg_password: str = Form(...)):
    existing_user = await users_collection.find_one({"username": reg_username})
    if existing_user:
        return templates.TemplateResponse(request=request, name="login.html", context={"error": "Username already exists! Please choose another.", "success": None})
    await users_collection.insert_one({"username": reg_username, "email": reg_email, "password": reg_password})
    return templates.TemplateResponse(request=request, name="login.html", context={"error": None, "success": "Account created successfully! Please sign in."})
...
async def read_form(request: Request, session: str = Cookie(None)):
    ...
    # Increased limit to 10,000 records
    logs_cursor = logs_collection.find().sort("timestamp", -1).limit(10000)
    logs = await logs_cursor.to_list(length=10000)
...
async def process_prediction(
    ...
    gender: str = Form(...), SeniorCitizen: int = Form(...), Partner: str = Form(...), Dependents: str = Form(...),
    tenure: int = Form(...), PhoneService: str = Form(...), MultipleLines: str = Form(...), InternetService: str = Form(...),
    OnlineSecurity: str = Form(...), OnlineBackup: str = Form(...), DeviceProtection: str = Form(...), TechSupport: str = Form(...),
    StreamingTV: str = Form(...), StreamingMovies: str = Form(...), Contract: str = Form(...), PaperlessBilling: str = Form(...),
    PaymentMethod: str = Form(...), MonthlyCharges: float = Form(...), TotalCharges: float = Form(...)
):
    if session != "authenticated":
        return RedirectResponse(url="/login", status_code=303)
    ...
    log_doc = {
        ...
        "gender": gender, "tenure": tenure, "contract": Contract,
    ...
    return templates.TemplateResponse(request=request, name="index.html", context={"prediction": prediction, "probability": probability, "suggestions": suggestions, "logs": logs, "success": None})

@app.post("/bulk-upload", response_class=HTMLResponse)
async def bulk_upload(request: Request, file: UploadFile = File(...), session: str = Cookie(None)):
    if session != "authenticated":
        return RedirectResponse(url="/login", status_code=303)
    try:
        ...
        for index, row in df.iterrows():
            ...
            prediction, probability = make_prediction(input_data)
            cust_id = str(row.get('customerID', row.get('customer_id', f"CUST-{random.randint(1000, 9999)}")))
        ...
        success_msg = f"Successfully processed and logged {processed_count} customer records with original IDs!"
    ...
    return templates.TemplateResponse(request=request, name="index.html", context={"prediction": None, "probability": None, "suggestions": None, "logs": logs, "success": success_msg})

@app.get("/clear-logs", response_class=HTMLResponse)
async def clear_logs(session: str = Cookie(None)):
    if session != "authenticated":
        return RedirectResponse(url="/login", status_code=303)
    await logs_collection.delete_many({})
    return RedirectResponse(url="/?success=All+bulk+and+prediction+logs+deleted+successfully", status_code=303)

@app.get("/download-cleaned-csv")
async def download_cleaned_csv(session: str = Cookie(None)):
    if session != "authenticated":
        return RedirectResponse(url="/login", status_code=303)
    ...
    if not logs:
        df = pd.DataFrame(columns=["customer_id", "timestamp", "gender", "tenure", "contract", "monthly_charges", "total_charges", "prediction", "probability"])
    else:
    ...
    response.headers["Content-Disposition"] = "attachment; filename=cleaned_customer_churn_predictions.csv"
    return response
